...
export const upsertChallengeProgress = async (challengeId: number) => {
  ...
  const responseChallenge = await fetch(
    `${nextLink}/api/challenges/${challengeId}`,
    {
      method: "GET",
    }
  );
  ...
  if (lessonId > 1) {
    const beforeResponseChallenge = await fetch(
      `${nextLink}/api/challenges/${challengeId - 1}`,
      {
        method: "GET",
      }
    );
    ...
    if (lessonId > beforeDataChallenge.lessonId) {
      const beforeLesson = await fetch(
        `${nextLink}/api/lessons/${lessonId - 1}`,
        {
          method: "GET",
        }
      );
      ...
      if (dataLesson.unitId > 1) {
        if (dataLesson.unitId > beforeLessonData.unitId) {
          const responseUnit = await fetch(
            `${nextLink}/api/units/${beforeLessonData.unitId}`,
            {
              method: "GET",
            }
          );
